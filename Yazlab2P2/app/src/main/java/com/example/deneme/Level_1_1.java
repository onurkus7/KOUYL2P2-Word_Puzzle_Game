package com.example.deneme;

import androidx.appcompat.app.AppCompatActivity;

import android.content.Intent;
import android.os.Bundle;
import android.os.CountDownTimer;
import android.view.View;
import android.widget.Button;
import android.widget.Chronometer;
import android.widget.TextView;

import java.util.ArrayList;
import java.util.List;
import java.util.Random;

public class Level_1_1 extends AppCompatActivity {

    Button btn_harf_n;
    Button btn_harf_v;
    Button btn_harf_e;
    Button btn_kontrol;
    Button btn_karistir;
    Button btn_sil;
    float pos_n_x,pos_n_y;
    float pos_v_x,pos_v_y;
    float pos_e_x,pos_e_y;
    TextView nev_n,nev_e,nev_v,en_e,ve_e;
    Chronometer chronometer;
    TextView puan_txt;
    TextView harfler_txt;
    int puan=0;
    int i=0,j=0,k=0;
    Long id;
    String [] cevaplar=new String[3];
    List<Button> btn_dizi=new ArrayList<Button>();
    List<String> harfler_dizi=new ArrayList<String>();
    List<String> seviye_karsilastir_dizi=new ArrayList<String>();
    List<Intent> next_level_dizi=new ArrayList<Intent>();
    Veritabani veritabani=new Veritabani(Level_1_1.this);


    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_level_1_1);

        chronometer=findViewById(R.id.chronometer_1_1);
        chronometer.start();

        new CountDownTimer(30000,1000){
            @Override
            public void onTick(long millisUntilFinished) {

            }

            @Override
            public void onFinish() {
                puan-=10;
                puan_txt.setText(""+puan);
            }
        }.start();

        puan_txt=findViewById(R.id.txt_1_1_puan);
        puan_txt.setText(""+puan);
        harfler_txt=findViewById(R.id.txt_1_1_basilan_harfler);

        nev_n=findViewById(R.id.txt_nev_n);
        nev_e=findViewById(R.id.txt_nev_e);
        nev_v=findViewById(R.id.txt_nev_v);
        en_e=findViewById(R.id.txt_en_e);
        ve_e=findViewById(R.id.txt_ve_e);

        cevaplar[0]="NEV";
        cevaplar[1]="VE";
        cevaplar[2]="EN";

        btn_harf_n=findViewById(R.id.btn_1_1_n);
        btn_harf_n.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                btn_dizi.add(btn_harf_n);
                harfler_dizi.add("N");
                harfler_txt.setText(harfler_dizi.toString());

            }
        });

        btn_harf_v=findViewById(R.id.btn_1_1_v);
        btn_harf_v.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                btn_dizi.add(btn_harf_v);
                harfler_dizi.add("V");
                harfler_txt.setText(harfler_dizi.toString());

            }
        });

        btn_harf_e=findViewById(R.id.btn_1_1_e);
        btn_harf_e.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                btn_dizi.add(btn_harf_e);
                harfler_dizi.add("E");
                harfler_txt.setText(harfler_dizi.toString());

            }
        });

        btn_karistir=findViewById(R.id.btn_1_1_karistir);
        btn_karistir.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                pos_n_x=btn_harf_n.getX();
                pos_n_y=btn_harf_n.getY();

                pos_v_x=btn_harf_v.getX();
                pos_v_y=btn_harf_v.getY();

                pos_e_x=btn_harf_e.getX();
                pos_e_y=btn_harf_e.getY();

                btn_harf_n.setX(pos_v_x);
                btn_harf_n.setY(pos_v_y);

                btn_harf_v.setX(pos_e_x);
                btn_harf_v.setY(pos_e_y);

                btn_harf_e.setX(pos_n_x);
                btn_harf_e.setY(pos_n_y);

            }
        });

        btn_sil=findViewById(R.id.btn_1_1_sil);
        btn_sil.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (!(btn_dizi.isEmpty())){
                    btn_dizi.remove(btn_dizi.size()-1);
                    harfler_dizi.remove(harfler_dizi.size()-1);
                    harfler_txt.setText(harfler_dizi.toString());
                }
            }
        });

        btn_kontrol=findViewById(R.id.btn_1_1_kontrol);
        btn_kontrol.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                if (btn_dizi.size()<2){
                    puan-=harfler_dizi.size()*100;
                }else if (btn_dizi.size()==2) {
                    if (btn_dizi.indexOf(btn_harf_e) == 0 && btn_dizi.indexOf(btn_harf_n) == 1) {
                        if (i == 0) {
                            cevaplar[2] = "";
                            puan += harfler_dizi.size() * 100;
                            i++;
                            en_e.setText("E");
                            nev_n.setText("N");
                        }
                    } else if (btn_dizi.indexOf(btn_harf_v) == 0 && btn_dizi.indexOf(btn_harf_e) == 1) {
                        if (j == 0) {
                            cevaplar[1] = "";
                            puan += harfler_dizi.size() * 100;
                            j++;
                            nev_v.setText("V");
                            ve_e.setText("E");
                        }
                    }else {
                        puan-=harfler_dizi.size()*100;
                    }
                }else if (btn_dizi.size()==3) {
                    if (btn_dizi.indexOf(btn_harf_n) == 0 && btn_dizi.indexOf(btn_harf_e) == 1 && btn_dizi.indexOf(btn_harf_v) == 2) {
                        if (k == 0) {
                            cevaplar[0] = "";
                            puan += harfler_dizi.size() * 100;
                            k++;
                            nev_n.setText("N");
                            nev_e.setText("E");
                            nev_v.setText("V");
                        }
                    }else{
                        puan-=harfler_dizi.size()*100;
                    }
                }else {
                    puan-=harfler_dizi.size()*100;
                }
                puan_txt.setText(""+puan);
                harfler_dizi.clear();
                btn_dizi.clear();

                if (cevaplar[0]==""&&cevaplar[1]==""&&cevaplar[2]==""){
                    String seviye="Level 1_1";
                    veritabani.Veri_ekle(seviye,puan);

                    seviye_karsilastir_dizi=veritabani.Veri_Listele();

                    if (!(seviye_karsilastir_dizi.toString().contains("Level 1"))){
                        Intent level_bir=new Intent(Level_1_1.this,Level_1.class);
                        next_level_dizi.add(level_bir);
                    }
                    if (!(seviye_karsilastir_dizi.toString().contains("Level 1_2"))){
                        Intent level_bir_iki=new Intent(Level_1_1.this,Level_1_2.class);
                        next_level_dizi.add(level_bir_iki);
                    }
                    if (!(seviye_karsilastir_dizi.toString().contains("Level 1_3"))){
                        Intent level_bir_uc=new Intent(Level_1_1.this,Level_1_3.class);
                        next_level_dizi.add(level_bir_uc);
                    }
                    if (!(seviye_karsilastir_dizi.toString().contains("Level 1_4"))){
                        Intent level_bir_dort=new Intent(Level_1_1.this,Level_1_4.class);
                        next_level_dizi.add(level_bir_dort);
                    }
                    if (!(seviye_karsilastir_dizi.toString().contains("Level 1_5"))){
                        Intent level_bir_bes=new Intent(Level_1_1.this,Level_1_5.class);
                        next_level_dizi.add(level_bir_bes);
                    }

                    /*if (!(seviye_karsilastir_dizi.toString().contains("Level 1"))){
                        Intent level_bir=new Intent(Level_1_1.this,Level_1.class);
                        next_level_dizi.add(level_bir);
                    }else if (!(seviye_karsilastir_dizi.toString().contains("Level 1_2"))){
                        Intent level_bir_iki=new Intent(Level_1_1.this,Level_1_2.class);
                        next_level_dizi.add(level_bir_iki);
                    }else if (!(seviye_karsilastir_dizi.toString().contains("Level 1_3"))){
                        Intent level_bir_uc=new Intent(Level_1_1.this,Level_1_3.class);
                        next_level_dizi.add(level_bir_uc);
                    }else if (!(seviye_karsilastir_dizi.toString().contains("Level 1_4"))){
                        Intent level_bir_dort=new Intent(Level_1_1.this,Level_1_4.class);
                        next_level_dizi.add(level_bir_dort);
                    }else if (!(seviye_karsilastir_dizi.toString().contains("Level 1_5"))){
                        Intent level_bir_bes=new Intent(Level_1_1.this,Level_1_5.class);
                        next_level_dizi.add(level_bir_bes);
                    }*/

                    if (!(seviye_karsilastir_dizi.isEmpty())) {
                        Random random = new Random();
                        int ras = random.nextInt(next_level_dizi.size());
                        startActivity(next_level_dizi.get(ras));
                    }/*else {
                        Intent bitti=new Intent(Level_1_1.this,BaslaActivity.class);
                        startActivity(bitti);
                    }*/
                    if (seviye_karsilastir_dizi.size()==5){
                        Intent bitti=new Intent(Level_1_1.this,BaslaActivity.class);
                        startActivity(bitti);
                    }
                }



            }
        });


    }
}
